...
describe('UsersService.create', () => {
  it('hashes the password before persisting', async () => {
    ...
    const repo = makeRepo({ createUser: vi.fn().mockResolvedValue(makeUser({ roleId })) });
    const service = new UsersService(repo);

    await service.create('tenant-1', { email: 'a@b.test', name: 'A', roleId, password: 'plaintext123' });

    const [, dataArg] = vi.mocked(repo.createUser).mock.calls[0]!;
    expect(dataArg.passwordHash).not.toBe('plaintext123');
    expect(await bcrypt.compare('plaintext123', dataArg.passwordHash)).toBe(true);
  });
});
...
describe('UsersService.get', () => {
  it('throws NotFoundError when the user does not exist', async () => {
    ...
    await expect(service.get('tenant-1', randomUUID())).rejects.toThrow(NotFoundError);
  });

  it('returns the user when found', async () => {
    const user = makeUser();
    const service = new UsersService(makeRepo({ findUserById: vi.fn().mockResolvedValue(user) }));
...
describe('UsersService.update', () => {
  it('throws NotFoundError when the repo reports no row updated', async () => {
    const service = new UsersService(makeRepo());
    await expect(service.update('tenant-1', randomUUID(), { name: 'X' })).rejects.toThrow(
      NotFoundError,
    );
  });
  ...
  it('returns the updated user on success', async () => {
    ...
    await expect(service.update('tenant-1', user.id, { name: 'Updated' })).resolves.toEqual(user);
  });
});
...
describe('UsersService.changePassword', () => {
  ...
  it('throws NotFoundError when the repo reports no row updated', async () => {
    const service = new UsersService(makeRepo());
    await expect(service.changePassword('tenant-1', 'user-1', 'x'.repeat(10))).rejects.toThrow(
      NotFoundError,
    );
  });
});
...
describe('UsersService.remove', () => {
  it('throws BadRequestError when a user tries to delete themself', async () => {
    ...
    await expect(service.remove('tenant-1', userId, userId)).rejects.toThrow(BadRequestError);
  });

  it('throws NotFoundError when the target user does not exist', async () => {
    const service = new UsersService(makeRepo());
    await expect(service.remove('tenant-1', randomUUID(), randomUUID())).rejects.toThrow(
      NotFoundError,
    );
  });
