...
describe("users.repo", () => {
  ...
  async function makeTenantWithRole() {
    const tenant = await adminPrisma.tenant.create({
      data: { slug: `u-${randomUUID()}`, name: "X" },
    });
    const role = await adminPrisma.role.create({
      data: { tenantId: tenant.id, name: "Member" },
    });
    return { tenant, role };
  }
  ...
  describe("updateUser", () => {
    ...
    it("returns null when the user does not exist", async () => {
      ...
      expect(
        await repo.updateUser(tenant.id, randomUUID(), { name: "X" }),
      ).toBeNull();
    });
  });
  ...
  describe("updatePassword", () => {
    it("updates the password hash", async () => {
      ...
      const raw = await adminPrisma.user.findUniqueOrThrow({
        where: { id: user.id },
      });
  ...
  describe("softDeleteUser", () => {
    it("is idempotent — deleting twice does not throw", async () => {
      ...
      await repo.softDeleteUser(tenant.id, user.id);
      await expect(
        repo.softDeleteUser(tenant.id, user.id),
