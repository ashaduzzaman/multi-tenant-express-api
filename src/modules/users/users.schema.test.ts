...
describe("createUserInput", () => {
  ...
  it("rejects an invalid email", () => {
    expect(createUserInput.safeParse({ ...valid, email: "nope" }).success).toBe(
      false,
    );
  });
  ...
  it("rejects a short password", () => {
    expect(
      createUserInput.safeParse({ ...valid, password: "short" }).success,
    ).toBe(false);
  });

  it("rejects a non-uuid roleId", () => {
    expect(
      createUserInput.safeParse({ ...valid, roleId: "not-a-uuid" }).success,
    ).toBe(false);
  });
});
...
describe("updateUserInput", () => {
  it("accepts a partial payload", () => {
    expect(updateUserInput.parse({ name: "New Name" })).toEqual({
      name: "New Name",
    });
  });
...
describe("changePasswordInput", () => {
  ...
  it("rejects a short password", () => {
    expect(changePasswordInput.safeParse({ password: "short" }).success).toBe(
      false,
    );
  });
});
