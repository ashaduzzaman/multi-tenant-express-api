...
import type { CreateUserData, PublicUser, UpdateUserFields } from './users.repo.js';
import type { CreateUserInput, UpdateUserInput } from './users.schema.js';

export interface UsersRepo {
  listUsers: (tenantId: string, params: PaginationParams) => Promise<PaginatedResult<PublicUser>>;
  findUserById: (tenantId: string, userId: string) => Promise<PublicUser | null>;
  ...
  updatePassword: (tenantId: string, userId: string, passwordHash: string) => Promise<boolean>;
...
export class UsersService {
  ...
  async list(tenantId: string, params: PaginationParams): Promise<PaginatedResult<PublicUser>> {
  ...
  async update(tenantId: string, userId: string, input: UpdateUserInput): Promise<PublicUser> {
  ...
  async changePassword(tenantId: string, userId: string, newPassword: string): Promise<void> {
    ...
    const updated = await this.repo.updatePassword(tenantId, userId, passwordHash);
    if (!updated) throw new NotFoundError('User not found');
  }

  async remove(tenantId: string, actingUserId: string, targetUserId: string): Promise<void> {
