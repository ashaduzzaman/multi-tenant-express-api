...
describe("RolesService.update", () => {
  it("throws NotFoundError when the role does not exist", async () => {
    ...
    await expect(
      service.update(randomUUID(), randomUUID(), { name: "X" }),
  ...
  it("invalidates the permission cache when permissionIds change", async () => {
    ...
    await permissionCache.getPermissions(tenantId, role.id, () =>
    ...
    const result = await permissionCache.getPermissions(
      tenantId,
      role.id,
      fetcher,
    );
...
describe("RolesService.remove", () => {
  ...
  it("throws NotFoundError when the role does not exist", async () => {
    const service = new RolesService(makeRepo());
    await expect(service.remove(randomUUID(), randomUUID())).rejects.toThrow(
      NotFoundError,
    );
  });
  ...
  it("throws ForbiddenError for a system role", async () => {
    ...
    await expect(service.remove(randomUUID(), role.id)).rejects.toThrow(
      ForbiddenError,
    );
  });
