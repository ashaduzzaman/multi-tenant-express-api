...
describe('roles.repo', () => {
  ...
  async function makeTenant() {
    return adminPrisma.tenant.create({ data: { slug: `roles-${randomUUID()}`, name: 'X' } });
  }

  async function permissionIds(names: string[]): Promise<string[]> {
    const rows = await adminPrisma.permission.findMany({ where: { name: { in: names } } });
  ...
  describe('createRole / findRoleById', () => {
    it('creates a role with permissions and returns it with stats', async () => {
      ...
      const role = await repo.createRole(tenant.id, { name: 'Support', permissionIds: ids });
    ...
    it('throws ConflictError for a duplicate name within the same tenant', async () => {
      ...
      await expect(repo.createRole(tenant.id, { name: 'Dup' })).rejects.toThrow(ConflictError);
    });

    it('allows the same role name in two different tenants', async () => {
      const t1 = await makeTenant();
      const t2 = await makeTenant();
      await expect(repo.createRole(t1.id, { name: 'SameName' })).resolves.toBeDefined();
      await expect(repo.createRole(t2.id, { name: 'SameName' })).resolves.toBeDefined();
    });
  ...
  describe('updateRole', () => {
    it('renames a role', async () => {
      ...
      const updated = await repo.updateRole(tenant.id, role.id, { name: 'New' });
      expect(updated.name).toBe('New');
    });

    it('replaces the full permission set when permissionIds is provided', async () => {
      const tenant = await makeTenant();
      const [readId, createId] = await permissionIds(['users:read', 'users:create']);
      const role = await repo.createRole(tenant.id, { name: 'R', permissionIds: [readId!] });

      const updated = await repo.updateRole(tenant.id, role.id, { permissionIds: [createId!] });
