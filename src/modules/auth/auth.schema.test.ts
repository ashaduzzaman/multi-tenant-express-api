...
describe('registerInput', () => {
  ...
  it('rejects an uppercase or symbol-containing slug', () => {
    expect(registerInput.safeParse({ ...valid, tenantSlug: 'Acme_Inc' }).success).toBe(false);
  });
...
describe('loginInput', () => {
  it('accepts a valid payload', () => {
    const valid = { tenantSlug: 'acme', email: 'owner@acme.test', password: 'anything' };
    expect(loginInput.parse(valid)).toEqual(valid);
  });

  it('rejects an empty password (but does not enforce a minimum length policy)', () => {
    expect(
      loginInput.safeParse({ tenantSlug: 'acme', email: 'owner@acme.test', password: '' })
        .success,
    ).toBe(false);
  });

  it('rejects a missing tenantSlug', () => {
    expect(loginInput.safeParse({ email: 'owner@acme.test', password: 'x' }).success).toBe(false);
  });
});
