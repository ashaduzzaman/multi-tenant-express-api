...
describe('auth router (e2e)', () => {
  ...
  it('registers a tenant + owner, setting access and refresh cookies', async () => {
    ...
    expect(cookies.find((c) => c.startsWith('mt_refresh='))).toMatch(/Path=\/api\/v1\/auth\/refresh/);
  });

  it('rejects registration with an invalid payload (422)', async () => {
    const res = await request(app)
      .post('/api/v1/auth/register')
      .send({ tenantName: '', tenantSlug: 'x', email: 'not-an-email', password: '123', name: '' });
  ...
  it('logs in with correct credentials and rejects incorrect ones identically', async () => {
    ...
    const good = await request(app)
      ...
      .send({ tenantSlug: payload.tenantSlug, email: payload.email, password: payload.password });
    expect(good.status).toBe(200);
    expect(good.body.data.user.email).toBe(payload.email);

    const bad = await request(app)
      .post('/api/v1/auth/login')
      .send({ tenantSlug: payload.tenantSlug, email: payload.email, password: 'wrong' });
    expect(bad.status).toBe(401);

    const unknownTenant = await request(app)
      .post('/api/v1/auth/login')
      .send({ tenantSlug: 'does-not-exist', email: payload.email, password: payload.password });
  ...
  it('supports the full register → me → refresh → logout → refresh-fails flow', async () => {
    ...
    const meRes = await request(app).get('/api/v1/auth/me').set('Cookie', cookies.join('; '));
    ...
    const refreshRes = await request(app)
      ...
      .set('Cookie', cookies.filter((c) => c.startsWith('mt_refresh=')).join('; '));
    ...
    const logoutRes = await request(app)
      ...
      .set('Cookie', newCookies.filter((c) => c.startsWith('mt_refresh=')).join('; '));
