...
import { pingTenantContext, withTenantContext, withTenantContextReadOnly } from './tenant-context.js';
...
describe('withTenantContext', () => {
  ...
  it('rejects a missing tenantId before touching the database', async () => {
    await expect(withTenantContext('', async (tx) => tx.tenant.findMany())).rejects.toThrow(
      TenantContextError,
    );
  });
  ...
  it('scopes SELECTs to only the given tenant — proves RLS isolation', async () => {
    ...
    const seenByA = await withTenantContext(tenantA.id, (tx) => tx.tenant.findMany());
    expect(seenByA.map((t) => t.id)).toEqual([tenantA.id]);

    const seenByB = await withTenantContext(tenantB.id, (tx) => tx.tenant.findMany());
    expect(seenByB.map((t) => t.id)).toEqual([tenantB.id]);
  });

  it('returns zero rows for a tenant id that matches no row', async () => {
    const seen = await withTenantContext(randomUUID(), (tx) => tx.tenant.findMany());
  ...
  it('rolls back and rethrows when the callback throws', async () => {
    ...
    await expect(
      withTenantContext(tenant.id, async (tx) => {
        await tx.tenant.update({ where: { id: tenant.id }, data: { name: 'mutated' } });
        throw new Error('boom');
      }),
    ).rejects.toThrow('boom');

    const reread = await adminPrisma.tenant.findUniqueOrThrow({ where: { id: tenant.id } });
  ...
  describe('withTenantContextReadOnly', () => {
    ...
    it('allows reads scoped to the tenant', async () => {
      ...
      const seen = await withTenantContextReadOnly(tenant.id, (tx) => tx.tenant.findMany());
    ...
    it('rejects a write attempted inside the read-only transaction', async () => {
      ...
      await expect(
        withTenantContextReadOnly(tenant.id, (tx) =>
          tx.tenant.update({ where: { id: tenant.id }, data: { name: 'nope' } }),
        ),
      ).rejects.toThrow();
    });
  });
