...
function fakeResponse(): { res: Response; status: ReturnType<typeof vi.fn>; json: ReturnType<typeof vi.fn> } {
...
describe('errorHandler', () => {
  ...
  it('maps a known AppError (< 500) to its own status code, without logging at error level', () => {
    const { res, status, json } = fakeResponse();
    errorHandler(new NotFoundError('Widget not found'), { requestId: 'req-2' } as Request, res, vi.fn());
  ...
  it('maps a 5xx AppError to its status code', () => {
    ...
    errorHandler(serviceUnavailable, { requestId: 'req-3' } as Request, res, vi.fn());
  ...
  it('maps an unknown error to a generic 500 without leaking internals', () => {
    const { res, status, json } = fakeResponse();
    errorHandler(new Error('some internal detail'), { requestId: 'req-4' } as Request, res, vi.fn());
