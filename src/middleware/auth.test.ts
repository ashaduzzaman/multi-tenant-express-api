...
async function validToken(overrides: Partial<Record<string, string>> = {}): Promise<{
  token: string;
  userId: string;
  tenantId: string;
  roleId: string;
  ...
  const token = await signAccessToken({ sub: userId, tid: tenantId, rid: roleId, email });
...
describe('authMiddleware', () => {
  ...
  it('calls next with UnauthorizedError for a malformed Bearer header', async () => {
    const req = { cookies: {}, headers: { authorization: 'Basic abc123' } } as unknown as Request;
    const next = vi.fn() as NextFunction;
